  ...
  public abstract static class BoundVisitor<R> extends ExpressionVisitor<R> {
    ...
    public <T> R predicate(BoundPredicate<T> pred) {
      if (pred.isLiteralPredicate()) {
        ...
        switch (pred.op()) {
          case LT:
            return lt(pred.term(), termPred.rightTerm());
          case LT_EQ:
            return ltEq(pred.term(), termPred.rightTerm());
          case GT:
            return gt(pred.term(), termPred.rightTerm());
          case GT_EQ:
            return gtEq(pred.term(), termPred.rightTerm());
          case EQ:
            return eq(pred.term(), termPred.rightTerm());
          case NOT_EQ:
            return notEq(pred.term(), termPred.rightTerm());
          default:
            throw new IllegalStateException(
                "Invalid operation for BoundTermPredicate: " + pred.op());
        }
